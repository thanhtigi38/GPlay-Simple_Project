
#if UNITY_EDITOR
using UnityEditor;
#endif
using UnityEngine;
using UnityEngine.UI;

//===============================================================
//Developer:  CuongCT
//Company:    ONESOFT
//Date:       2017
//================================================================

[ExecuteInEditMode]
[RequireComponent(typeof(ILayoutElement))]
public class UILocalize : LocalizeBehaviour
{
	/// <summary>
	/// Localization key.
	/// </summary>

	public string key;

	/// <summary>
	/// Manually change the value of whatever the localization component is attached to.
	/// </summary>

	public string value
	{
		set
		{
			if (!string.IsNullOrEmpty(value))
			{
                ILayoutElement w = GetComponent<ILayoutElement>();
                Text lbl = w as Text;
                Image sp = w as Image;

				if (lbl != null)
				{
                    // If this is a label used by input, we should localize its default value instead
                    InputField input = lbl.GetComponentInParent<InputField>();
					if (input != null && input.textComponent == lbl) input.text = value;
					else lbl.text = value;
#if UNITY_EDITOR
					if (!Application.isPlaying) EditorUtility.SetDirty(lbl);
#endif
				}
				else if (sp != null)
				{
     //               Button btn = sp.FindInParents<Button>(sp.gameObject);
					//if (btn != null && btn.tweenTarget == sp.gameObject)
					//	btn.normalSprite = value;

					//sp.spriteName = value;
					//sp.MakePixelPerfect();
#if UNITY_EDITOR
					if (!Application.isPlaying) EditorUtility.SetDirty(sp);
#endif
				}
			}
		}
	}

	bool mStarted = false;

	/// <summary>
	/// Localize the widget on enable, but only if it has been started already.
	/// </summary>

	void OnEnable ()
	{
#if UNITY_EDITOR
		if (!Application.isPlaying) return;
#endif
		if (mStarted) OnLocalize();
	}

	/// <summary>
	/// Localize the widget on start.
	/// </summary>

	void Start ()
	{
#if UNITY_EDITOR
		if (!Application.isPlaying) return;
#endif
		mStarted = true;
		OnLocalize();
	}

	/// <summary>
	/// This function is called by the Localization manager via a broadcast SendMessage.
	/// </summary>

	public override void OnLocalize ()
	{
		// If no localization key has been specified, use the label's text as the key
		if (string.IsNullOrEmpty(key))
		{
			return;
			Text lbl = GetComponent<Text>();
			if (lbl != null) key = lbl.text;
		}

		// If we still don't have a key, leave the value as blank
		if (!string.IsNullOrEmpty(key)) value = Localization.Get(key);
	}
}
